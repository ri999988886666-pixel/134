...
| 3 | `git add` | Добавление файлов в индекс | `.`, `-p`, `-A` | `git add .` | ![alt text](image-2.png) |
| 4 | `git commit` | Фиксация изменений | `-m`, `-a`, `--amend` | `git commit -m "message"` | ![alt text](image-3.png) |
| 5 | `git status` | Показать состояние рабочей директории | `-s`, `-b` | `git status` | ![alt text](image-4.png) |
| 6 | `git push` | Отправка изменений на удаленный репозиторий | `-u`, `--force` | `git push origin main` | ![alt text](image-5.png) |
| 7 | `git pull` | Загрузка изменений с удаленного репозитория | `--rebase`, `--ff-only` | `git pull origin main` | ![alt text](image-6.png) |
| 8 | `git fetch` | Загрузка изменений без слияния | `--all`, `--prune` | `git fetch origin` | ![alt text](image-7.png) |
| 9 | `git branch` | Работа с ветками | `-a`, `-d`, `-v` | `git branch feature/new` | ![alt text](image-8.png) |
...
| 12 | `git diff` | Показать изменения | `--staged`, `HEAD~1` | `git diff HEAD~1` | ![alt text](image-11.png) |
| 13 | `git log` | История коммитов | `--oneline`, `--graph` | `git log --oneline` | ![alt text](image-12.png) |
| 14 | `git reset` | Сброс изменений | `--soft`, `--hard` | `git reset --soft HEAD~1` | ![alt text](image-13.png) |
| 15 | `git revert` | Отмена коммита | `-n`, `--no-edit` | `git revert abc123` | ![alt text](image-14.png) |
