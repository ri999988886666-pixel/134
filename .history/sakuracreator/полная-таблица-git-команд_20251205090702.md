...
| 4 | `git commit` | Фиксация изменений | `-m`, `-a`, `--amend` | `git commit -m "message"` |  |
| 5 | `git status` | Показать состояние рабочей директории | `-s`, `-b` | `git status` |  |
| 6 | `git push` | Отправка изменений на удаленный репозиторий | `-u`, `--force` | `git push origin main` |  |
...
| 9 | `git branch` | Работа с ветками | `-a`, `-d`, `-v` | `git branch feature/new` |  |
| 10 | `git checkout` | Переключение между ветками | `-b`, `-f` | `git checkout -b feature` |  |
| 11 | `git merge` | Слияние веток | `--no-ff`, `--squash` | `git merge feature` |  |
| 12 | `git diff` | Показать изменения | `--staged`, `HEAD~1` | `git diff HEAD~1` |  |
| 13 | `git log` | История коммитов | `--oneline`, `--graph` | `git log --oneline` |  |
...
| 18 | `git remote` | Управление удаленными репозиториями | `add`, `-v`, `remove` | `git remote add origin url` |  |
| 19 | `git show` | Показать информацию о коммите | `--stat`, `--name-only` | `git show HEAD` |  |
| 20 | `git rm` | Удаление файлов | `--cached`, `-r` | `git rm file.txt` |  |
| 21 | `git mv` | Перемещение файлов | (нет) | `git mv old.txt new.txt` |  |
| 22 | `git clean` | Очистка неотслеживаемых файлов | `-f`, `-d`, `-n` | `git clean -fd` |  |
| 23 | `git grep` | Поиск текста в файлах репозитория | `-n`, `-i` | `git grep "TODO"` |  |
| 24 | `git rebase` | Перебазирование ветки | `-i`, `--continue` | `git rebase -i HEAD~3` |  |
| 25 | `git cherry-pick` | Применение конкретного коммита | `-n`, `-x` | `git cherry-pick abc123` |  |
| 26 | `git bisect` | Бинарный поиск бага | `start`, `bad`, `good` | `git bisect start` |  |
| 27 | `git blame` | Авторство строк файла | `-L`, `-w` | `git blame file.txt` |  |
| 28 | `git config` | Настройка параметров Git | `--global`, `--list` | `git config user.name` |  |
| 29 | `git help` | Помощь по командам | (команда) | `git help commit` |  |
| 30 | `git archive` | Создание архива | `--format=zip` | `git archive -o repo.zip HEAD` |  |
| 31 | `git describe` | Описать коммит | `--tags`, `--always` | `git describe --tags` |  |
| 32 | `git shortlog` | Сводка по авторам | `-sn`, `-e` | `git shortlog -sn` |  |
| 33 | `git whatchanged` | Что изменилось | `-p`, `--since` | `git whatchanged --since="1 week"` |  |
| 34 | `git reflog` | История ссылок | `show`, `expire` | `git reflog show` |  |
